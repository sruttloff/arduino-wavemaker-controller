2 Jebao Wavemaker (SW-4) Pumpen selbst steuern
=======
Da das Strömungsbild trotz der vielen Möglichkeiten die die Controller von Hause aus bieten, nicht meinen Wünschen entsprach, habe ich dieses kleine Projekt auf die Beine gestellt.

Zur Umsetzung benötigt wird:
1. Arduino Mega 2560 (eine kleiner Version des Arduino kann verwendet werden, dann muss allerdings die Pinbelegung angepasst werden)
2. 1 x OP Verstärker LM358
3. 6 x 19kOhm Widerstände
4. 2 x 22uF Elko's
5. Netzteil mit 9-12V DC
6. Ein paar Meter 2 Adriges Kabel um die Controller der Pumpen anzuschliessen.

Hier der Schaltplan:

![Schaltplan](https://github.com/kenpratt/pumpkin/blob/master/circuit-breadboard_Schaltplan.png?raw=true)

Hier der Aufbau auf einer Steckplatine (Breadboard):

![Schaltplan](https://github.com/kenpratt/pumpkin/blob/master/circuit-breadboard_Steckplatine.png?raw=true)

